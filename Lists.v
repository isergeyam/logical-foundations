...
Fixpoint alternate' (l1 l2 : natlist) (lef : bool) : natlist :=
  match l1, l2 with 
  | nil, l2 => l2
  | l1, nil => l1
  | h :: t, h' :: t' => match lef with 
    | true => h :: h' :: (alternate' t t' true)
    | false => h' :: h :: (alternate' t t' false)
  end
  end.

Definition alternate (l1 l2 : natlist) : natlist :=
  alternate' l1 l2 true.
...
Fixpoint count' (v acc : nat) (s : bag) : nat :=
  match s with 
  | nil => acc
  | h :: t => if h =? v then count' v (S acc) t else count' v acc t
  end.

Fixpoint count (v : nat) (s : bag) : nat :=
  match s with 
  | nil => 0
  | cons n s' => if n =? v then 1 + count v s' else count v s'
  end.
  
Theorem count_empty: forall n : nat,
  count n [] = 0.
Proof.
  intros n. 
  unfold count. 
...
Fixpoint remove_one' (v : nat) (s acc : bag) : bag :=
  match s with 
  | nil => acc
  | h :: t => if h =? v then sum acc t else remove_one' v t (h :: acc)
  end.
...
Fixpoint remove_all' (v : nat) (s acc : bag) : bag :=
  match s with 
  | nil => acc
  | h :: t => if h =? v then remove_all' v t acc
    else remove_all' v t (h :: acc)
  end.
...
Fixpoint drop_until (v : nat) (s : bag) : bag := 
  match s with 
  | nil => nil
  | h :: t => if v =? h then t else drop_until v t
  end.
...
Theorem bag_count_add: forall (b : bag) (n : nat),
...
Proof.
  intros s. 
  induction s as [|s' n IH]. 
  - simpl. 
    reflexivity. 
  -  simpl. 
    destruct (s' =? 0) as [] eqn:E. 
    + rewrite leb_n_Sn. 
      reflexivity. 
    + simpl. 
      rewrite E. 
...
(* Lemma rev_empty: forall (l : natlist),
  [] = rev l -> l = [].
Proof.
  intros l. 
  simpl. 
