...
Inductive bevalR: bexp -> bool -> Prop :=
  ...
  | E_BAnd e1 e2 b1 b2: e1 ==>b b1 -> e2 ==>b b2 -> BAnd e1 e2 ==>b b1 && b2
...
Proof.
  ...
  subst. destruct b; discriminate.
  rewrite Heqloopdef in IHcontra2. apply IHcontra2. reflexivity. 
Qed.
(** [] *)
...
Inductive no_whilesR: com -> Prop :=
  | NO_Skip : no_whilesR CSkip 
  | NO_Ass x a : no_whilesR (CAss x a)
  | NO_Seq c1 c2 (H1: no_whilesR c1) (H2: no_whilesR c2) : no_whilesR (CSeq c1 c2)
  | NO_If b c1 c2 (H1: no_whilesR c1) (H2: no_whilesR c2) : no_whilesR (CIf b c1 c2)
.
...
Proof.
  ...
  - intros H. induction c; try constructor;
    try (simpl in H; apply andb_true_iff in H; destruct H as [H1 H2]; 
    apply IHc1 in H1; apply IHc2 in H2); try apply H1; try apply H2.
    simpl in H. discriminate. 
  - intros H. induction H; try reflexivity;
    simpl; rewrite andb_true_iff; split; 
    try apply IHno_whilesR1; apply IHno_whilesR2. 
Qed.
(** [] *)
...
Proof.
  ...
  - intros. remember (IHno_whilesR1 st) as H1. destruct H1 as [st' H1]. 
    remember (IHno_whilesR2 st') as H2. destruct H2 as [st'' H2]. 
    exists st''. apply E_Seq with st'. apply H1. apply H2. 
  - intros. remember (IHno_whilesR1 st) as H1. destruct H1 as [st' H1]. 
    remember (IHno_whilesR2 st) as H2. destruct H2 as [st'' H2]. 
    destruct (beval st b) eqn:E. exists st'. apply E_IfTrue. apply E. apply H1. 
...
Definition get_top_two (l : list nat) :=
  match l with 
    | h :: t => match t with 
      | h' :: t' => (h,h',t',true)
      | [] => (0,0,[],false)
...
Fixpoint s_execute (st : state) (stack : list nat)
  ...
  match prog with 
    ...
    | SPlus :: p => match (get_top_two stack) with
      | (h,h',t,b) => if b then s_execute st (h' + h :: t) p
                      else s_execute st stack p
    end
    | SMinus :: p => match (get_top_two stack) with
      | (h,h',t,b) => if b then s_execute st (h' - h :: t) p
                      else s_execute st stack p
    end
    | SMult :: p => match (get_top_two stack) with
      | (h,h',t,b) => if b then s_execute st (h' * h :: t) p
                      else s_execute st stack p
    end
  end.
...
Proof.
  intros st p1. induction p1 as [|c p1 IH]. 
  - intros. reflexivity. 
  - destruct c; try (simpl; intros; apply IH);
    simpl; intros; destruct (get_top_two stack); destruct p; destruct p; 
    destruct b; apply IH; apply IH. 
Qed.
...
Proof.
  ...
  intros; simpl; rewrite execute_app; remember (IHe1 stack) as H1; rewrite H1; 
    rewrite execute_app; remember (IHe2 (aeval st e1 :: stack)) as H2; rewrite H2;
    reflexivity. 
Qed.
...
Inductive com : Type :=
  ...
  | CIf (b : bexp) (c1 c2 : com)
  | CWhile (b : bexp) (c : com)
  | CFor (init : com) (test : bexp) (after : com) (body : com).
...
Notation "'while' x 'do' y 'end'" :=
         (CWhile x y)
            (in custom com at level 89, x at level 99, y at level 99) : com_scope.
Notation "'for' init , test , after 'do' body 'end'" :=
    (CFor init test after body) (in custom com at level 89, init at level 99, test at level 99, after at level 99,
      body at level 99) : com_scope. 
...
Inductive ceval : com -> state -> result -> state -> Prop :=
  ...
  | E_SeqContinue: forall c1 c2 r st st' st'',
      st =[ c1 ]=> st' / SContinue ->
      st' =[ c2 ]=> st'' / r ->
      st =[ c1; c2 ]=> st'' / r
  ...
  | E_IfFalse : forall st st' b c1 c2 r2,
      beval st b = false ->
      st =[ c2 ]=> st' / r2 ->
      st =[ if b then c1 else c2 end]=> st' / r2
  | E_WhileFalse : forall b st c,
      beval st b = false ->
  ...
  | E_WhileTrueBreak : forall st st' b c,
      ...
      st  =[ c ]=> st' / SBreak ->
      st  =[ while b do c end ]=> st' / SContinue
  | E_ForFalse : forall st init test after body,
    beval st test = false ->
    st =[ for init, test, after do body end ]=> st / SContinue
  | E_ForTrueContinue: forall st st' st'' init test after body,
    beval st test = true ->
    st =[ body ]=> st' / SContinue ->
    st' =[ for init, test, after do body end ]=> st'' / SContinue ->
    st =[ for init, test, after do body end ]=> st'' / SContinue
...
Theorem while_stops_on_break : forall b c st st',
  ...
  st =[ while b do c end ]=> st' / SContinue.
Proof.
  intros. apply E_WhileTrueBreak. apply H. apply H0. 
Qed.
(** [] *)
...
Proof.
  ...
  injection Heqc'. rewrite H2 in H. rewrite H in H0. discriminate. 
  apply IHceval2 in Heqc'. apply Heqc'. assumption. rewrite <- H4. 
...
Proof.
  ...
  induction H; try (intros; inversion H0; subst; split; reflexivity). 
  intros. inversion H1. subst. apply IHceval1 in H4. destruct H4 as [H4 H5]. 
  subst. apply IHceval2 in H8. assumption. 
  apply IHceval1 in H7. destruct H7 as [_ contra]. discriminate. 
  intros. inversion H0. apply IHceval in H3. destruct H3 as [_ contra]. discriminate. 
  subst.  apply IHceval in H6. assumption. 
  intros. inversion H1. apply IHceval in H9. assumption. subst. rewrite H8 in H. discriminate. 
  intros. inversion H1. rewrite H8 in H. discriminate. 
  apply IHceval in H9. assumption. 
  intros. inversion H0. subst. split; reflexivity. rewrite H in H3. discriminate. 
  rewrite H in H3. discriminate. 
  intros. inversion H2. subst. rewrite H8 in H. discriminate. 
  subst. apply IHceval1 in H6. destruct H6 as [H6 _]. subst. auto.
