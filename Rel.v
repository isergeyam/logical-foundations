...
Proof.
  unfold partial_function. intros x y1 y2 H. 
  destruct H. 
Qed.
...
Proof.
  ...
  induction o as [| o'].
  - inversion Hmo. 
  - assert (H: m <= S m). {apply le_S. apply le_n. } 
    assert (H1: S n <= S m). { apply (le_trans (S n) m (S m) Hnm H). } 
    apply (le_trans (S n) (S m) (S o') H1 Hmo). 
Qed.
(** [] *)
...
Lemma Sn_neq_n: forall n, 
  ~ (S n = n).
Proof.
  intros n H. 
  induction n as [|n IH]. 
  - discriminate H. 
  - injection H as H. apply IH in H. apply H. 
Qed.
...
Proof.
  ...
  induction n as [|n IH]. 
  - inversion H. 
  - inversion H. apply Sn_neq_n in H1. apply H1. 
    assert (H2: S n <= S (S n)). {apply le_S. apply le_n. } 
    assert (S n <= n). {apply (le_trans (S n) (S(S n)) n H2 H1). } 
...
Proof.
  ...
  assert (H1: 0 <= 1). {apply le_S. apply le_n. } 
...
Proof.
  ...
  - intros b H1 H2. destruct b as [|b]. inversion H1. 
    apply le_S_n in H1. apply le_S_n in H2. f_equal. apply (IH b H1 H2). 
Qed.
(** [] *)
