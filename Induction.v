...
Proof.
  ...
  - reflexivity.
  - rewrite -> IHn'.
    rewrite -> Basics.negb_involutive .
    reflexivity.
Qed.
(** [] *)
...
Lemma mult_comm_helper_l: forall m n : nat,
  m * S n = m + m * n.
Proof.
  intros m n. 
...
Lemma mult_comm_helper_r: forall m n : nat,
  S m * n = m * n + n.
Proof.
  intros m n. 
  destruct m as [| m'].
...
Proof.
  ...
  - rewrite -> mult_0_l. 
    rewrite -> mult_0_r. 
    reflexivity.
  - rewrite -> mult_comm_helper_l. 
    rewrite -> mult_comm_helper_r. 
    rewrite -> IHm'. 
    rewrite -> plus_comm. 
    reflexivity.
Qed.
...
Lemma S_a_plus_b: forall a b : nat,
  S (a + b) = (S a) + b.
Proof.
  intros [] b.
  - simpl. 
...
(* nat_to_bin (bin_to_nat (B0 Z)) = Z != BO Z *)
...
Example test_normalize3: normalize (B1 (B0 (B0 Z))) = B1 Z.
Proof. simpl. reflexivity. Qed.

Example test_normalize4: 
  normalize (B1 (B0 (B0 Z))) = nat_to_bin(bin_to_nat (B1 (B0 (B0 Z)))).
Proof. simpl. reflexivity. Qed.

Lemma bin_nat_B0: forall b : bin,
...
Lemma incr_B0: forall b : bin,
  B0 (incr b) = incr(incr(B0 b)).
Proof.
  intros [|b|b].
  - simpl. 
    reflexivity.
...
Lemma nat_bin_B1: forall n : nat,
  B1 (nat_to_bin n) = nat_to_bin (2 * n + 1).
Proof.
  induction n as [| n IHn].
  - simpl. 
    reflexivity.
...
Lemma normalize_incr: forall b : bin,
...
Lemma nat_bin_B0: forall n : nat,
  normalize (B0 (nat_to_bin n)) = nat_to_bin (2 * n).
Proof.
  induction n as [|n IHn].
  - simpl. 
    reflexivity.
  - rewrite -> nat_to_bin_incr. 
    ...
    rewrite <- nat_to_bin_incr. 
    rewrite <- nat_to_bin_incr. 
    assert (H: S(S(2*n)) = 2 * (S n)).
    {simpl. rewrite <- plus_n_O. rewrite -> S_a_plus_b. 
    rewrite -> plus_comm. reflexivity. }
    rewrite -> H. 
    reflexivity. 
Qed.

Lemma normalize_idemp : forall b : bin,
...
Proof.
  ...
  - simpl. 
    reflexivity.
  - simpl. 
    destruct (normalize b) as [|b'|b'] eqn:E. 
    + simpl. 
      reflexivity.
    + assert (H: normalize (B0 (B0 b')) = 
    match (normalize (B0 b')) with 
    | Z => Z
    | b => B0 b
    end).
    { simpl. reflexivity. } 
    ...
    reflexivity.
    + simpl. 
      assert (H: normalize (B1 b') = B1 (normalize b')).
      { simpl. reflexivity. } 
      rewrite <- H. 
      rewrite -> IHb. 
      reflexivity.
...
Lemma normalize_B0 : forall b : bin,
  normalize (B0 b) = normalize (B0 (normalize b)).
Proof.
  induction b as [|b IHb|b IHb].
  - simpl. 
    reflexivity.
