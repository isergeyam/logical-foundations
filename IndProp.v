...
Proof.
  ...
  - simpl. apply Hm. 
  - simpl. apply ev_SS in IH. apply IH. 
Qed.
(** [] *)
...
Proof.
  ...
  - intros H. 
    ...
    + generalize dependent IHm. generalize dependent IHn. apply ev_sum. 
  - intros H. 
    induction H as [|n E IH]. 
    + apply ev'_0. 
    + assert (Hp : 2 + n = S(S n)). { reflexivity. }
      rewrite <- Hp. 
      rewrite plus_comm. 
      apply (ev'_sum n 2 IH ev'_2). 
Qed.
(** [] *)
...
Theorem plus_4_comm_assoc: forall a b c d : nat,
  a + b + (c + d) = a + c + b + d.
Proof.
  intros a b c d.
  assert (H: b + (c + d) = c + b + d).
...
Proof.
  ...
  assert (Hnmp : ev ((n + m) + (n + p))). 
  { 
    generalize dependent Hnp.  
    generalize dependent Hnm. 
    apply ev_sum. 
  }
  assert (H: n + m + (n + p) = n + n + m + p). { apply plus_4_comm_assoc. } 
  assert (H1: n + n + m + p = (n + n) + (m + p)). { rewrite -> plus_assoc. reflexivity. }  
  rewrite H1 in H. 
  rewrite H in Hnmp. 
  assert (Hnn : ev (n + n)). { rewrite ev_even_iff. exists n. symmetry. apply double_plus. } 
  generalize dependent Hnn. 
  generalize dependent Hnmp. 
  apply ev_ev__ev. 
Qed.
(** [] *)
...
Theorem test_total: forall n m : nat, total_relation n m.
Proof.
  intros n m. 
  apply total_rel. 
Qed.
...
Theorem test_empty: forall n m : nat, ~ empty_relation n m.
Proof.
  intros n m He. 
  inversion He. 
Qed.
...
Proof.
  intros n1 n2 m H. 
  split. 
  - assert (Hn: n1 <= n1 + n2). {apply le_plus_l. } 
    apply (le_trans n1 (n1 + n2) m Hn H). 
  - rewrite plus_comm in H. 
    assert (Hn: n2 <= n2 + n1). {apply le_plus_l. } 
    apply (le_trans n2 (n2 + n1) m Hn H). 
Qed.
...
Proof.
  intros n. 
  induction n as [|n IH]. 
  - intros p m q. left. apply O_le_n. 
  - intros m p q H. 
    destruct p as [|p]. 
    + assert (H1: 0 + q = q). {reflexivity. } 
      rewrite H1 in H. 
      apply plus_le in H. 
      right. 
...
Proof.
  unfold lt. 
  intros n m H. 
  apply (le_trans (S n) m (S m) H (le_n_Sn m)). 
Qed.
...
Proof.
  unfold lt. 
  intros n1 n2 m H. 
  assert (H1: S(n1 + n2) = n1 + (S n2)). { apply plus_n_Sm. }
  assert (H2: S(n1 + n2) = n2 + (S n1)). { rewrite plus_comm. apply plus_n_Sm. } 
  split. 
  - rewrite H2 in H. 
    apply plus_le in H. 
    destruct H as [_ H]. 
...
Proof.
  ...
  - destruct m as [|m]. 
    ...
    + simpl. intros H. apply IH in H. apply n_le_m__Sn_le_Sm. apply H. 
Qed.
...
Proof.
  intros n. 
  induction n as [|n IH]. 
  - simpl. intros m H. reflexivity. 
  - intros m H. 
    inversion H. 
    + symmetry. apply leb_refl. 
    + simpl. 
      assert (H2: n <= m0). {apply (le_trans n (S n) m0 (le_n_Sn n) H0). } 
      apply IH in H2. 
      apply H2.
Qed.

Lemma leb_iff_le: forall n m, n <=? m = true <-> n <= m. 
Proof.
...
Lemma plus_n_m_O: forall n m : nat, n + m = 0 -> n = 0 /\ m = 0. 
Proof.
  intros [] [] H. 
  - split; reflexivity. 
...
Proof.
  ...
  - intros H. 
    induction H as []. 
    + reflexivity. 
    + rewrite plus_comm. rewrite <- plus_n_Sm. f_equal. rewrite plus_comm. apply IHR. 
    + rewrite <- plus_n_Sm. f_equal. apply IHR. 
    + simpl in IHR. assert (H1: m + S n = S (m + n)). {symmetry. apply plus_n_Sm. } 
      rewrite H1 in IHR. 
...
Inductive subseq {X: Type} : list X -> list X -> Prop :=
  | subseq1 l : subseq [] l
  | subseq2 h l1 l2 (H2: subseq l1 l2) : subseq (h :: l1) (h :: l2)
  | subseq3 h1 l1 h2 l2 (H: subseq (h1 :: l1) l2) : subseq (h1 :: l1) (h2 :: l2)
  .
...
Proof.
  ...
  induction H as []. 
  - apply subseq1. 
  - assert (H1: (h :: l2) ++ l3 = h :: (l2 ++ l3)). { reflexivity. } 
    rewrite H1. 
    apply (subseq2 h l1 (l2 ++ l3) IHsubseq). 
  - assert (H1: (h2 :: l2) ++ l3 = h2 :: (l2 ++ l3)). { reflexivity. } 
    rewrite H1. 
    apply (subseq3 h1 l1 h2 (l2 ++ l3) IHsubseq). 
Qed.
...
Lemma app_nil_nil: forall X (l1 l2 : list X), 
  l1 ++ l2 = [] -> l1 = [] /\ l2 = [].
Proof.
  intros X [] [] H. 
  - split; reflexivity. 
...
Lemma rex_exp_char: forall T (s : list T) (t : T),
 s =~ Char t -> s = [t]. 
Proof.
  intros T s t H. 
  inversion H. 
...
Proof.
  ...
  - generalize dependent s1. 
    induction s2 as [|t s2 IH]. 
    + simpl. intros s1 H. inversion H. reflexivity. 
    + intros s1 H. simpl in H. destruct s1 as [|t' s1]. 
      * inversion H. 
        apply app_nil_nil in H1. 
        destruct H1 as [H1 _]. 
        rewrite H1 in H3. 
        inversion H3.  
      * inversion H. 
        apply IH in H4. 
        apply rex_exp_char in H3. 
        rewrite H3. 
        rewrite H4. 
        reflexivity. 
  - generalize dependent s2. 
    induction s1 as [|t s1 IH]. 
    + intros s2 H. 
      rewrite <- H. 
      simpl. 
      apply MEmpty. 
    + intros s2 H. 
      ...
      * injection H as H H1. 
        apply IH in H1. 
        simpl. 
        assert (H2: t :: s1 = [t] ++ s1). {reflexivity. } 
        rewrite H2. 
        apply MApp. 
        {rewrite H. apply MChar. } 
        apply H1.
Qed.
(** [] *)
...
Lemma or_true: forall b : bool, b || true = true.
...
Proof.
  ...
  - intros [s Hmatch]. 
    ...
    + simpl. rewrite IHHmatch. apply or_true. 
    + reflexivity. 
    + reflexivity. 
  - intros H. 
    induction re as []. 
...
Proof.
  ...
  - exists []. 
    split. 
    + reflexivity. 
    + intros s HIn. simpl in HIn. destruct HIn. 
  - assert (E': re0 = re). {injection E as E. apply E. } 
    apply IHexp_match2 in E. 
    destruct E as [ss' [E1 E2]]. 
    exists (s1 :: ss'). 
    split. 
    + simpl. rewrite E1. reflexivity. 
    + intros s. simpl. intros [H1 | H2]. 
    { rewrite <- E'. rewrite <- H1. apply H. }
    { apply E2 in H2. apply H2. }
Qed.
(** [] *)
...
Proof.
  ...
  - simpl. apply H2. 
  - apply IHexp_match2 in E.  
  { rewrite <- app_assoc. apply MStarApp. apply H1_.  apply E. } 
  { apply H2.  }
Qed.
...
Proof.
  ...
  induction m as [|m IH]. 
  - simpl. apply MStar0. 
  - simpl. apply app_star. apply H. apply IH. 
Qed.

Lemma le_O_n: forall n, n <= 0 <-> n = 0.
Proof.
  split. 
  - intros H. inversion H. reflexivity. 
  - intros H. rewrite H. apply le_n. 
Qed.

Lemma length_O: forall T (l : list T), length l <> 0 <-> l <> []. 
Proof.
  intros T l. split. 
  - intros H contra. assert (H1: l = [] -> length l = 0). { intros h. rewrite h. reflexivity.  } 
    apply H1 in contra. apply H in contra. apply contra. 
  - intros H contra. assert (H1: l = []). 
  { induction l as [|l n IH]. reflexivity. simpl in contra. discriminate contra. }
    apply H in H1. apply H1. 
Qed.

Lemma le_O_neq: forall n, 1 <= n <-> n <> 0. 
Proof.
  intros n. split. 
  - intros H contra. rewrite contra in H. inversion H. 
  - intros H. induction n as [|n IHn]. 
  {assert (H1: 0 = 0). { reflexivity. } apply H in H1. destruct H1. } 
  destruct n as [|n']. { apply le_n.  } 
  assert (H1: S n' <> 0). { intros contra. discriminate contra.  } 
  apply IHn in H1. apply (le_trans 1 (S n') (S (S n')) H1 (le_S (S n') (S n') (le_n (S n')))). 
Qed.
...
Proof.
  ...
  - (* MApp *)
    ...
    + apply IH1 in H1. destruct H1 as [s1' [s2' [s3' [H1 [H2 H3]]]]]. 
      exists s1'. exists s2'. exists (s3' ++ s2). rewrite H1. split. 
      ...
      { split. apply H2.  intros m. rewrite app_assoc. rewrite app_assoc. apply MApp.  
        ...
        - apply Hmatch2. }
    + apply IH2 in H2. destruct H2 as [s1' [s2' [s3' [H1 [H2 H3]]]]]. 
      exists (s1 ++ s1'). exists s2'. exists s3'. rewrite H1. split. 
      { rewrite app_assoc. rewrite app_assoc. reflexivity. } 
      { split. apply H2.  intros m. rewrite <- app_assoc. apply MApp. 
        - apply Hmatch1.
        - apply H3. }
  - (* MUnionL *)
    simpl. intros H. apply plus_le in H. destruct H as [H1 _]. apply IH in H1. 
    destruct H1 as [s1' [s2' [s3' [H1 [H2 H3]]]]]. 
      exists s1'. exists s2'. exists s3'. split. apply H1. split. apply H2. 
      intros m. apply MUnion'. left. apply H3. 
  - (* MUnionR *)
    simpl. intros H. apply plus_le in H. destruct H as [_ H2]. apply IH in H2. 
    destruct H2 as [s1' [s2' [s3' [H1 [H2 H3]]]]]. 
    exists s1'. exists s2'. exists s3'. split. apply H1. split. apply H2. 
    intros m. apply MUnion'. right. apply H3. 
  - (* MStar0 *)
    simpl. intros H. apply le_O_n in H. apply pumping_constant_0_false in H. destruct H. 
  - (* MStarApp *)
    simpl. intros H. exists []. exists (s1 ++ s2). exists []. simpl. rewrite app_nil_r. 
    split. reflexivity. assert (H1: 1 <= length(s1 ++ s2)). 
    {apply (le_trans 1 (pumping_constant re) (length (s1 ++ s2)) (pumping_constant_ge_1 T re) H). } 
    apply le_O_neq in H1. apply length_O in H1. split. apply H1. intros m. rewrite app_nil_r. 
    apply napp_star'. apply MStarApp. apply Hmatch1. apply Hmatch2. 
Qed. 
(** [] *)
...
Proof.
  intros P b H. split. 
  - intros p. inversion H. reflexivity. {apply H0 in p. destruct p. }
  - intros H1. inversion H. apply H0. {rewrite <- H2 in H1. discriminate. }
Qed.
(** [] *)
...
Proof.
  intros n l H contra. 
  induction l as [|n' l IH]. 
  - simpl in contra. apply contra. 
  - simpl in contra.  simpl in H. 
    destruct (eqbP n n') as [H1 | H1]. 
    + simpl in H. discriminate H. 
    + simpl in H. destruct contra as [contra | contra]. symmetry in contra. apply H1 in contra. destruct contra. 
      apply IH in H. apply H. apply contra. 
Qed.
(** [] *)
...
Inductive nostutter {X:Type} : list X -> Prop :=
  ...
  | NoStutterOne h : nostutter [h]
  | NoStutterApp h h' t (H1: h <> h') (H2: nostutter (h' :: t)) : nostutter (h :: h' :: t)
.
...
Inductive in_order_merge {X:Type} : list X -> list X -> list X -> Prop :=
  | InOrderNil l : in_order_merge [] l l
  | InOrderApp h l1 l2 l (H: in_order_merge l1 l2 l) : in_order_merge (h :: l1) l2 (h :: l)
  | InOrderSwap l1 l2 l (H: in_order_merge l1 l2 l) : in_order_merge l2 l1 l.

Lemma In_filter: forall X (l l1: list X) test,
  filter test l = l1 -> forall x, In x l1 -> test x = true. 
Proof.
  intros X l l1 test H. 
  generalize dependent l1. 
  induction l as [|n l IH]. 
  - simpl. intros l1 H. rewrite <- H. simpl. intros x contra. destruct contra. 
  - simpl. destruct (test n) as [] eqn:E. 
    + intros l1 H x H1. destruct l1 as [|n' l1]. simpl in H1. destruct H1. 
      injection H as H H0. simpl in H1. destruct H1 as [H1 | H1]. 
      rewrite <- H1. rewrite <- H. apply E. 
      apply (IH l1 H0). apply H1. 
    + apply IH. 
Qed.

Theorem filter_iff_in_order: forall X (l l1: list X) test,
  filter test l = l1 <-> (forall x, In x l1 -> test x = true) /\ 
    exists (l2 : list X), (forall x, In x l2 -> test x = false) /\ in_order_merge l1 l2 l.  
Proof.
  intros X l l1 test. split. 
  - intros H. split. apply (In_filter X l l1 test H). 
    remember (fun x => notb (test x)) as test' eqn:E. 
    remember (filter test' l) as l2 eqn:E2. 
    symmetry in E2. 
    assert (H1: forall x, test x = false <-> test' x = true). 
    { intros x. destruct (test x) as [] eqn:E'. rewrite E. rewrite E'. 
    simpl. split.  intros contra. discriminate contra. 
    intros contra. discriminate contra. 
    split. rewrite E. rewrite E'. simpl. intros _. reflexivity. intros _. reflexivity. 
    }
    exists l2. split. 
    {intros x. rewrite H1. apply (In_filter X l l2 test' E2). } 
    generalize dependent l1. 
    generalize dependent l2. 
    induction l as [|n l IH]. 
    + intros l2 E2 l1 H. simpl in H. simpl in E2. rewrite <- H. rewrite <- E2. apply InOrderNil. 
    + simpl. intros l2 H2 l1 H. destruct (test n) as [] eqn:E3. 
      rewrite E in H2. rewrite E3 in H2. simpl in H2. rewrite <- E in H2. 
      destruct l1 as [|n' l1]. discriminate H. injection H as H H0. rewrite H. apply InOrderApp. 
      apply IH. apply H2. apply H0. 
      rewrite E in H2. rewrite E3 in H2. simpl in H2. rewrite <- E in H2. 
      destruct l2 as [|n' l2]. discriminate H2. injection H2 as H2 H0.
      rewrite H2. apply InOrderSwap. apply InOrderApp. apply InOrderSwap. apply IH. 
      apply H0. apply H. 
  - admit.
Admitted.
(* Do not modify the following line: *)
...
Theorem filter_iff_subseq: forall X (l l1: list X) test,
  filter test l = l1 <-> forall (l2: list X), (subseq l2 l /\
  forall x, In x l2 -> test x = true) -> subseq l2 l1. 
Proof.
Admitted.
...
Proof.
  ...
  - reflexivity. 
  - reflexivity. 
  - simpl. rewrite rev_app_distr. simpl. rewrite IHpal at 1. reflexivity. 
Qed.
(* Do not modify the following line: *)
...
Inductive len {X: Type}: list X -> nat -> Prop := 
  | NilLen: len [] 0
  | AppLen h t n (H: len t n) : len (h :: t) (S n). 

Lemma len_length: forall X (l: list X), len l (length l).
Proof.
  intros X l. 
  induction l as [|x l]. 
  - simpl. apply NilLen. 
  - simpl. apply AppLen. apply IHl. 
Qed.

Lemma iff_neq_iff: forall (A B: Prop), (A <-> B) -> (~A <-> ~B). 
Proof.
  intros A B.
  intros [H1 H2]. split. apply contrapositive. apply H2. apply contrapositive. apply H1. 
Qed.

Lemma length_O_iff: forall X (l : list X), length l = 0 <-> l = []. 
Proof.
  intros x l. split. 
  - intros H. destruct l as [|l n]. reflexivity. simpl in H. discriminate H. 
  - intros H. rewrite H. reflexivity. 
Qed.

Lemma app_mono_left_one: forall X x (l1 l2 : list X), l1 ++ [x] = l2 ++ [x] -> l1 = l2. 
Proof.
  intros X x l1. 
  induction l1 as [|n l1 IH]. 
  - intros l2 H. simpl in H. destruct l2 as [|n' l2]. reflexivity. simpl in H. injection H as _ H1. 
  assert (H2: length (@nil X) = length (l2 ++ [x])). {f_equal. apply H1. } 
  simpl in H2. rewrite app_length in H2. rewrite plus_comm in H2. simpl in H2. discriminate H2. 
  - intros l2 H. destruct l2 as [|n' l2]. simpl in H. injection H as _ H. apply app_nil_nil in H. destruct H as [H H1]. 
    discriminate H1. injection H as H H1. rewrite H. apply IH in H1. rewrite H1. reflexivity. 
Qed.

Lemma app_mono_left: forall X (l3 l1 l2 : list X), l1 ++ l3 = l2 ++ l3 -> l1 = l2. 
Proof.
  intros X l3. 
  induction l3 as [|n l3 IH]. 
  - intros l1 l2 H. rewrite app_nil_r in H. rewrite app_nil_r in H. apply H. 
  - intros l1 l2 H. assert (H1: n :: l3 = [n] ++ l3). {reflexivity. } rewrite H1 in H. 
    rewrite app_assoc in H. rewrite app_assoc in H. apply IH in H. apply app_mono_left_one in H. apply H. 
Qed.

Theorem rev_pal' : 
  forall {X} (n : nat) (l : list X), length l <= n ->  l = rev l ->  pal l.
Proof.
  intros X n. 
  induction n as [|n IH]. 
  - intros l H. inversion H. rewrite length_O_iff in H1. rewrite H1. intros _. apply PalNil. 
  - intros l H H1. destruct l as [|n' l]. apply PalNil. simpl in H1. 
    destruct (rev l) as [|n'' l'] eqn:E. simpl in H1. injection H1 as H1. rewrite H1. apply PalOne. 
    rewrite H1. injection H1 as H1 H2. rewrite H1. apply PalApp. assert (H3: rev (rev l) = rev (n'' :: l')). 
    {f_equal. apply E. } rewrite rev_involutive in H3. simpl in H3. rewrite H3 in H2. rewrite H1 in H2. 
    apply app_mono_left in H2. symmetry in H2. simpl in H. assert (H4: length l <= n). 
    {apply Sn_le_Sm__n_le_m. apply H. } assert (H5: length l = length (rev l' ++ [n''])). 
    {f_equal. apply H3. } rewrite app_length in H5. simpl in H5. rewrite plus_comm in H5. simpl in H5. 
    rewrite <- H2 in H5. assert (H6: length l' <= n). 
    {apply (le_trans (length l') (length l) n). rewrite H5. apply le_S. apply le_n. apply H4. } 
    apply IH in H6. apply H6. apply H2. 
Qed.

Theorem rev_pal: forall X (l: list X), l = rev l -> pal l.
Proof.
  intros X l H. 
  apply (rev_pal' (length l) l). apply le_n. apply H. 
Qed. 
(* FILL IN HERE
...
Proof.
  ...
  - simpl in H. destruct H as [H1 | H2]. { exists []. exists l. simpl. rewrite H1. reflexivity. }
  apply IH in H2. destruct H2 as [l1 [l2 H2]]. exists (n :: l1). exists l2. simpl. rewrite H2. reflexivity. 
Qed.
...
Inductive repeats {X:Type} : list X -> Prop :=
  | RepeatsIn h t (H: In h t) : repeats (h :: t)
  | Repeats h t (H: repeats t) : repeats (h :: t)
.
...
Proof.
  ...
  - simpl. intros  l _ _ contra. inversion contra. 
  - intros l2 Hexcl H1 H2. assert (H: In x l1' \/ ~ In x l1'). {apply Hexcl. } 
    destruct H as [H | H]. apply (RepeatsIn x l1' H). assert (H3: In x l2). 
    {apply H1. simpl. left. reflexivity. } apply in_split in H3. destruct H3 as [l1'' [l2'' H3]]. 
    remember (l1'' ++ l2'') as l3 eqn:E. apply (Repeats x l1'). apply (IHl1' l3) in Hexcl. apply Hexcl. 
    { intros x0 H0. assert (H4: In x0 l2). {apply H1. simpl. right. apply H0. } rewrite H3 in H4. 
    simpl in H4. apply In_app_iff in H4. simpl in H4. rewrite E. apply In_app_iff. 
    destruct H4 as [H4 | [H4 | H4]]. left. apply H4. rewrite <- H4 in H0. apply H in H0. destruct H0. 
    right. apply H4. }
    unfold lt in *. simpl in H2. apply Sn_le_Sm__n_le_m in H2. 
    assert (H4: length l2 = length (l1'' ++ x :: l2'')). {f_equal. apply H3. } 
    rewrite app_length in H4. simpl in H4. rewrite <- plus_n_Sm in H4. rewrite <- app_length in H4. 
    rewrite <- E in H4. rewrite <- H4. apply H2. 
Qed.
...
Proof.
  intros. split. intros. apply app_exists in H. destruct H as [s0 [s1 H]]. 
  destruct s0 as [|a' s0]. simpl in H. left. destruct H as [H1 H]. rewrite H1. apply H. 
  right. simpl in H. destruct H as [H H1]. injection H as H H0. exists s0. exists s1. 
  split. apply H0. rewrite H. apply H1. 
Admitted.
(** [] *)
