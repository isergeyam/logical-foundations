...
Proof.
  ...
  - intros p. apply HRQ in p. apply HQP in p. apply p. 
Qed.
(** [] *)
...
Proof.
  ...
  - induction l as [|x l IH]. 
    ...
    + simpl. intros [H | H]. 
      ...
      * apply IH in H. destruct H as[x' [Ex' EIn]]. exists x'. split. apply Ex'. right. apply EIn. 
  ...
  - intros l'' a. split. 
    ...
    * intros [H | H]. 
      + simpl. simpl in H. destruct H as [H | H]. 
        { left. apply H. } { right. apply IH. left. apply H. } 
...
Proof.
  ...
  - split. 
    ...
    + simpl. intros HH x contra. destruct contra. 
  - split. 
    + intros H. simpl. split. 
      * apply H. simpl. left. reflexivity. 
      * rewrite <- IH. intros x HIn. apply H. simpl. right. apply HIn. 
    + intros H x HIn. simpl in HIn. destruct HIn as [H1 | H2]. 
      * simpl in H. destruct H as [Ha Hx]. rewrite <- H1. apply Ha. 
      * simpl in H. destruct H as [Ha Hx]. rewrite <- IH in Hx. apply Hx in H2. apply H2. 
Qed.
(** [] *)
...
Proof.
  ...
  - unfold combine_odd_even. 
    rewrite E. 
    assert (H: false = false). {reflexivity. } 
    apply Heven in H. 
    apply H. 
Qed.
...
Proof.
  ...
  - simpl. rewrite <- IH. unfold tr_rev. simpl. 
    assert (H: rev_append l [n] = rev_append l ([] ++ [n])). { reflexivity. } 
    rewrite H. 
...
Proof.
  ...
  - split. 
    ...
    + intros [contra | contra]. discriminate contra. discriminate contra. 
Qed.
(** [] *)
...
Proof.
  ...
  - simpl. destruct l2 as [|m l2]. 
    + split. 
      * intros H2. discriminate H2. 
      * intros H2. discriminate H2. 
    + destruct (eqb n m) as [] eqn:E. 
      * simpl. apply H in E. rewrite E. rewrite IH. split. 
      {intros H2. f_equal. apply H2. }
      {intros H2. injection H2 as H3. apply H3. } 
      * simpl. split. 
      { intros H2. discriminate H2. }
      { intros H2. injection H2 as H2 H3. apply H in H2. rewrite H2 in E. discriminate E. }
Qed.
...
Proof.
  intros X test l. 
  induction l as [|n l IH]. 
  - simpl. split. { intros H1. apply I. } { intros H1. reflexivity. } 
  - simpl. destruct (test n) as []. 
  { simpl. split. { intros H. split. reflexivity. apply IH in H. apply H. } { intros [_ H]. apply IH in H. apply H. }} 
  { simpl. split. { intros H. discriminate H. } { intros [contra _]. discriminate contra. }  } 
Qed.
...
Proof.
  intros excl. 
  unfold excluded_middle in excl. 
  intros X P H x. 
  assert (H1 : P x \/ ~ (P x)). 
  { apply excl. } 
  destruct H1 as [H1 | H1]. 
  - apply H1. 
  - assert (Ex : exists x : X, ~ P x). 
    { exists x. apply H1. } 
    apply H in Ex. 
    exfalso. 
    apply Ex. 
...
Proof.
  ...
  assert (PP: P \/ ~ P). 
  { apply excl. } 
  destruct PP as [TP | FP]. 
  - apply TP. 
  - assert (PQ : P -> Q). { apply not_implies_our_not. apply FP. } 
    apply H in PQ. 
    apply PQ. 
Qed.

Theorem peirce_double_negation: peirce -> double_negation_elimination. 
Proof.
  unfold peirce. 
  unfold double_negation_elimination. 
  intros Ppeirce P NNP. 
  unfold not in NNP. 
  assert (PNP : (P -> False) -> P). { apply not_implies_our_not. unfold not. apply NNP. } 
  apply Ppeirce in PNP. 
  apply PNP.
Qed.

Theorem double_negation_excluded : double_negation_elimination -> excluded_middle. 
Proof.
  unfold double_negation_elimination. 
  unfold excluded_middle. 
  intros Pdneg P. 
  assert (goal: ~~(P \/ ~P)). {apply excluded_middle_irrefutable. } 
  apply Pdneg in goal. 
  apply goal. 
Qed.
...
Proof.
  ...
  intros Pexcl P Q H. 
  assert (PNP : P \/ ~P). {apply Pexcl. } 
  destruct PNP as [p | np]. 
  - left. apply p. 
  - assert (QNQ : Q \/ ~Q). { apply Pexcl. } 
    destruct QNQ as [q | nq]. 
    + right. apply q. 
    + assert (NH : ~P /\ ~Q). { split. apply np. apply nq. } 
      apply H in NH. 
      destruct NH. 
Qed.

Theorem de_morgan_negation: de_morgan_not_and_not -> double_negation_elimination. 
Proof.
  unfold de_morgan_not_and_not. 
  unfold double_negation_elimination. 
  intros Pmorgan P NNp.  
  assert (H: ~(~~P /\ ~P) -> ~P \/ P). { apply Pmorgan. } 
  assert (H1: ~(~~P /\ ~P)). {apply not_both_true_and_false'. } 
  apply H in H1. 
  destruct H1 as [np | p]. 
...
Theorem de_morgan_implies: de_morgan_not_and_not -> double_negation_elimination -> implies_to_or. 
Proof.
  unfold de_morgan_not_and_not. 
  unfold implies_to_or. 
  intros Pmorgan Pdneg P Q PQ. 
  assert (Ngoal: P /\ ~Q -> False). 
  { intros [p nq]. apply PQ in p. apply nq in p. destruct p. }
  assert (PP : P <-> ~~P). {split. apply double_neg. apply Pdneg. } 
  rewrite PP in Ngoal. 
  apply Pmorgan in Ngoal. 
  apply Ngoal. 
Qed.

Theorem implies_to_or_excluded: implies_to_or -> excluded_middle. 
Proof.
  unfold implies_to_or. 
  unfold excluded_middle. 
  intros Pimpl P. 
  assert (H: P -> P). { intros p. apply p. } 
  apply Pimpl in H. 
  destruct H as [Np | p]. 
  - right. apply Np. 
  - left. apply p. 
